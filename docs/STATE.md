...
**875 unit tests**, up from 811. `pnpm lint`, `pnpm typecheck` and `pnpm build`
clean. Migrations `0008` and `0009` applied to a freshly reset test database.

### Review — what was found and what was done

The `reviewer` pass found **4 high, 5 medium and 8 low**, and separately traced
clean: case (a) of the prompt byte-identical to the `783a957` version (diffed
directly, not read); the sign convention end to end for a correctly-signed
credit; that a NULL discount reduces exactly to the previous arithmetic; that
the second opinion cannot loop or exceed one extra call; that `escalated`
accounting stays honest; that the Luhn scrub still covers the new fields and
that an admin-edited prompt cannot reach it; that the API key never leaves in a
return value, a log or a message; and authorization on every new query. All 17
findings are fixed.

Two of the four highs are the same mistake in two places — a predicate written
twice — and both were demonstrated rather than argued:

| Sev      | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Fix                                                                                                                                                                                                                                                                                                                                     |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **High** | **The held email was never released under its own default gate.** The hold asked "any flags?"; the release asked "no flags AND no missing fields?". They agree only under the stricter gate, which is not the default. So clearing the last warning removed the hold and never fired the release — while the page said "the automatic email is waiting until these are resolved" and the warnings it pointed at were already gone. Most receipts have an unread field, so this was the common case.                                                                                                                           | One predicate in `packages/shared/src/emailGate.ts`, used by both, with the release defined as the EDGE across it. The old tests missed it because the fixture had no missing fields — the one shape where the two rules agree.                                                                                                         |
| **High** | **`added_desc` dropped rows and `added_asc` looped forever.** `created_at` is a microsecond `timestamptz`; the cursor went through a millisecond JS `Date`. Reproduced at a two-millisecond spread — what a batch upload produces: **7 rows in, 5 returned** under `desc`; under `asc` the predicate was true of the cursor row itself, so the page repeated until the 40-iteration guard.                                                                                                                                                                                                                                    | The `admin.logs` fix again — `to_char` to a pinned UTC string, bound back as `::timestamptz`, never parsed in JS. **This is D-46's H-1 exactly, three days later, in code whose own commit message cited it.** The new fixture spaces rows 200µs apart; the old one used whole seconds, which is precisely where truncation is a no-op. |
| **High** | **A failed second opinion failed a receipt that pass 1 read fine.** The new call was unwrapped, unlike `retryIfItemsDoNotReconcile` thirty lines above whose header states the rule. A 529 on the confirmation read threw, BullMQ retried the whole job, pass 1 was paid for twice more, and the receipt ended `failed` with no data. The date trigger fires on ANY receipt over a week old, so a shoebox of old receipts ran the whole batch through it.                                                                                                                                                                     | Wrapped; a failed second opinion leaves pass 1 intact and `dateUnconfirmed` false.                                                                                                                                                                                                                                                      |
| **High** | **`transaction_discount` was write-only.** Stored, summed into `arithmetic_mismatch_total`, and absent from the read projection and every screen. A $10 credit made subtotal, tax and total visibly fail to add up with nothing to explain the gap — and if the model read the credit wrong there was no field to correct it.                                                                                                                                                                                                                                                                                                 | On the read path and on the receipt, shown when there is one.                                                                                                                                                                                                                                                                           |
| Med      | **The corrective re-read DELETED order-level credits.** `arithmeticHint` still taught only the pre-D-47 resolution ("drop the separate discount lines"), so a model that had emitted a coupon as a line item did exactly that and the credit vanished from the record — then `arithmetic_mismatch_total` fired on the receipt it had just broken.                                                                                                                                                                                                                                                                             | The hint carries both resolutions, and says explicitly not to discard the credit.                                                                                                                                                                                                                                                       |
| Med      | **A same-model date check replaced the whole reading.** A confirmation read taken to check one field overwrote merchant, items and confidence with a second sample of no greater authority — the inverse of the rule stated 300 lines above it.                                                                                                                                                                                                                                                                                                                                                                               | A date-only confirmation answers the date question and leaves pass 1 as the reading.                                                                                                                                                                                                                                                    |
| Med ×3   | The export's reconciliation block omitted the credit, so one coupon made a correct export's "Difference" row read $10.00 — on the sheet that calls itself the export's claim to be arithmetically sound. `SpendByCategory` subtracted a filtered item total from an UNFILTERED project total, so a one-month view of a $50k project rendered ~$49k "unitemised". Nothing enforced the credit's sign.                                                                                                                                                                                                                          | The discount is a term in the delta and its own summary row; the filtered total is used; the value is clamped to at most zero.                                                                                                                                                                                                          |
| Low ×8   | `taxIncluded` was passed to a function that never read it, under a comment asserting it mattered; the date carry-forward mutated the object `extraction_raw` is serialised from; a `date_unconfirmed` flag could survive onto a receipt whose date had become null; a malformed cursor was a 500 rather than a 400 (and wrote a `system.internal_error` row); the "waiting to be sent" line showed on instances with receipt emails off; `refreshModelCatalog`'s comment claimed a lock it does not take; a non-reconciling receipt cost three calls where two would do; `dateIsStale` compares a local date to a UTC cutoff. | All fixed except the last, which is noted — the threshold is a heuristic and a day either side of a seven-day boundary changes nothing.                                                                                                                                                                                                 |

The lesson worth keeping from the two highs: **both were one idea written
twice.** "Is this receipt settled" existed as a hold rule and a release rule;
"what is this row's sort key" existed as a column and a JS `Date`. Neither
duplicate was flagged by a type, and both passed their tests — because the
tests were written from the same understanding as the code.
