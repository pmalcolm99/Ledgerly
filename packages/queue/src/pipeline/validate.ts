...
/**
 * packages/queue/src/pipeline/validate.ts — sanity checks (task 6.7,
 * ARCHITECTURE.md §6.3). Never fails the receipt — each check independently
 * sets `extraction_status='partial'` and appends a flag to
 * `validation_flags`, because CLAUDE.md requires extraction to always save
 * *something*. Money comparisons go through `parseMoney` (integer cents),
 * never float arithmetic, matching D-21.
 */
